...
var registerBasicsSuccess = function(resultData){
	...
	if(resultData != "false"){
		...
		$.ajax({
			url: "${ctx}/customer/registerHouse.do"
			,type: "POST"
			,data: {
...
var registerHouseSuccess = function(resultData){
	if(resultData != "false"){
		...
		$.ajax({
			url: "${ctx}/customer/registerOffice.do"
			,type: "POST"
			,data: {
				customer_no: resultData
...
var registerOfficeSuccess = function(resultData){
	if(resultData != "false"){
		...
		$.ajax({
			url: "${ctx}/customer/registerThird.do"
			,type: "POST"
			,data: {
				customer_no: resultData
...
var registerThirdSuccess = function(resultData){
	if(resultData != "false"){
		...
		$.ajax({
			url: "${ctx}/customer/registerAdd.do"
			,type: "POST"
			,data: {
				customer_no: resultData
...
var registerAddSuccess = function(resultData){
	if(resultData != "false"){
		...
		$.ajax({
			url: "${ctx}/customer/registerAntiMoney.do"
			,type: "POST"
			,data: {
				customer_no: resultData
			...
			<div id="tabs">
						...
						<ul>
							<li><span class="note1 pointer">(*)거래구분</span>
								<select title="거래구분" id="거래구분" style="background: #ffffcc;" onchange="dealDivision();">
									<option value="01" selected="selected">1: 등록</option>								
									...
									<table summary="자택주소정보">
										...
										<tbody>
											<tr>
												...
												<td colspan="5">
                                                    <input type="text" id="tab1_zip_code" name="zip_code" style="background:#ffffcc; width:80px;" >
                                                    ...
                                                    <input type="text" name="address_before" id="tab1_address_before" style="background: #ffffcc; width:480px;">
                                                </td>
												<th>상세주소</th>
				...
				<div class="btn_both">
					<div class="fr change changebutton">
						<input type="button" value="저장" id="register">
					</div>					
				</div>
